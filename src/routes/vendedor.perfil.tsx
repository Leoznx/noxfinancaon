...
function Perfil() {
  ...
  async function salvarTelefone() {
    ...
    if (profileUpdate.error || internalUpdate.error) {
      toast.error(
        profileUpdate.error?.message || internalUpdate.error?.message || "Não foi possível salvar.",
      );
      return;
    }
      ...
      <div className="max-w-4xl space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-neutral-500">
                Suas informações de vendedor NOX.
              </p>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <Estado
            ...
            descricao="Não encontramos dados de perfil para o usuário logado."
          />
        ) : (
          !erro && (
            <>
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="grid gap-3 md:grid-cols-2">
                    <div>
                      <Label>Nome</Label>
                      <Input value={profile?.nome ?? internal?.full_name ?? ""} disabled />
                    </div>
                    <div>
                      <Label>E-mail</Label>
                      <Input value={profile?.email ?? internal?.email ?? ""} disabled />
                    </div>
                    <div>
                      <Label>Cargo</Label>
                      <Input value={internal?.role ?? "vendedor"} disabled />
                    </div>
                    <div>
                      <Label>Status</Label>
                      <div className="mt-2">
                        <Badge>{internal?.status ?? profile?.status ?? "ativo"}</Badge>
                      </div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Input value="Definidas pelo administrador na Equipe NOX" disabled />
                    </div>
                  </div>
                  <div className="flex flex-col gap-2 md:flex-row md:items-end">
                    ...
                    <Button onClick={salvarTelefone} disabled={saving}>
                      Salvar telefone
                    </Button>
                  </div>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent className="flex flex-col gap-2 md:flex-row md:items-end">
                  ...
                  <Button onClick={trocarSenha} disabled={saving}>
                    Alterar senha
                  </Button>
                </CardContent>
              </Card>
