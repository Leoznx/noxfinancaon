...
function CommissionsPage() {
  ...
  const load = useCallback(async (showRefreshState = false) => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : "Não foi possível carregar suas comissões.");
    } finally {
      setLoaded(true);
      setRefreshing(false);
    }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "seller_commissions" }, refreshSilently)
      .on("postgres_changes", { event: "*", schema: "public", table: "seller_client_partnerships" }, refreshSilently)
      .on("postgres_changes", { event: "*", schema: "public", table: "apolices" }, refreshSilently)
      .on("postgres_changes", { event: "*", schema: "public", table: "faturas_inquilino" }, refreshSilently)
      .on("postgres_changes", { event: "*", schema: "public", table: "mensalidades" }, refreshSilently)
      .subscribe();
        ...
        {errorMessage && (
          <div role="alert" className="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
            ...
            <div>
              ...
              <p className="mt-0.5 text-red-700">{errorMessage} Use o botão Atualizar para tentar novamente.</p>
            </div>
          </div>
        )}
