      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Gerencie colaboradores internos, cargos, permissões e desempenho comercial da equipe
            NOX.
          </p>
        </div>
...
/* ===================== CARGOS E PERMISSÕES ===================== */
// Módulos de ação (afetam telas/recursos) + módulos de navegação (controlam
// o que aparece no menu lateral e o acesso à rota — ver DashboardLayout.tsx
// e ProtectedRoute.tsx). Lista unificada com tudo que já existe hoje em
// role_permissions, pra grade nunca esconder uma permissão já configurada.
const MODULOS = [
  // ação
  "aprovacoes",
  "contratos",
  ...
  "metas",
  "pipeline",
  // navegação (controla visibilidade de aba + acesso à rota)
  "dashboard_admin",
  "consultas",
  "corretores",
  "usuarios",
...
export function TabPermissoes() {
  ...
  const toggle = async (role: string, mod: string, acao: string, value: boolean) => {
    ...
    const antes = existing?.[acao] ?? false;
    if (existing) {
      const { error } = await supabase
      ...
      const { error } = await supabase
        .from("role_permissions" as any)
        .insert({ role, module: mod, [acao]: value });
      if (error) {
        toast.error(error.message);
        return;
      }
    ...
    registrarAuditoria({
      actorUserId: user?.id,
      actorRole: user?.internalRole || user?.role,
      action: `alterar_permissao:${acao}`,
      tableName: "role_permissions",
      recordId: existing?.id,
      before: { role, module: mod, [acao]: antes },
      after: { role, module: mod, [acao]: value },
    });
    carregar();
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          O Admin possui acesso total ao sistema. Os cargos abaixo controlam apenas acessos dos
                ...
                <TableRow key={mod}>
                  ...
                  {ACOES.map((a) => (
                    <TableCell key={a} className="text-center">
                      <Switch
                        checked={!!row?.[a]}
                        onCheckedChange={(v) => toggle(selectedRole, mod, a, v)}
                      />
                    </TableCell>
                  ))}
                </TableRow>
...
export function TabEquipeComercial() {
    ...
    const enriched = await Promise.all(
      ((vendedores as any[]) ?? []).map(async (v) => {
        ...
        const { count: leads } = await supabase
          ...
          .select("id", { count: "exact", head: true })
          .eq("assigned_seller_id", v.id);
        const { data: comm } = await supabase
          .from("seller_commissions" as any)
          .select("status, reserve_amount, released_amount")
          .eq("seller_id", v.id)
          .eq("month", m)
          .eq("year", y);
        const reservas =
          (comm as any[] | null)?.reduce((s, r) => s + Number(r.reserve_amount ?? 0), 0) ?? 0;
        const estornos =
          (comm as any[] | null)?.filter((r) => r.status === "estornada").length ?? 0;
  ...
  const sorted = useMemo(
    () =>
      [...rows].sort(
        (a, b) => (b.perf?.contracts_activated ?? 0) - (a.perf?.contracts_activated ?? 0),
      ),
    [rows],
  );
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Performance, metas, comissões, reservas e ranking dos vendedores.
          </p>
        </div>
        <Button size="sm" onClick={materializar}>
          Materializar mês
        </Button>
      </CardHeader>
      <CardContent>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Últimas 200 ações registradas dos colaboradores internos.
        </p>
      </CardHeader>
      <CardContent>
        <Table>
