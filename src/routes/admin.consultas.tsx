...
export const Route = createFileRoute("/admin/consultas")({
  ...
  errorComponent: ({ error, reset }) => (
    <RouteErrorFallback error={error} reset={reset} message="Não foi possível carregar as consultas." />
  ),
});
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data, error } = await supabase
        ...
        .select("id, status, created_at, tenant_name, tenant_document, property_address, rent_value, role_solicitante, profile_id_solicitante")
    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-neutral-500 mt-2 font-medium">Consultas já analisadas (aprovadas ou reprovadas). Pendentes ficam em Aprovações.</p>
        </div>

        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1 max-w-md">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" size={18} />
            <Input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar por inquilino, CPF ou endereço..." className="pl-10 h-11" />
          </div>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={8} className="text-center py-16 text-neutral-500">Nenhuma consulta encontrada.</TableCell></TableRow>
              ) : filtradas.map((c) => (
                <TableRow key={c.id} className="hover:bg-neutral-50/50">
                  ...
                  <TableCell className="text-xs text-neutral-500 max-w-[220px] truncate">{c.property_address ?? "—"}</TableCell>
                  ...
                  <TableCell className="text-right pr-6">
                    <a href={`/consultas/${c.id}/resultado`}>
                      <Button size="sm" variant="ghost" className="gap-1"><Eye size={14} /> Ver</Button>
                    </a>
                  </TableCell>
                </TableRow>
              ))}
...
function StatusBadge({ status }: { status: string }) {
  ...
  const labels: Record<string, string> = { aprovado: "Aprovado", pendente: "Pendente", reprovado: "Reprovado" };
  return <Badge className={map[status] ?? "bg-neutral-100 text-neutral-700"}>{labels[status] ?? status}</Badge>;
}
