...
const COBERTURAS = [
  {
    id: "incendio",
    nome: "Incêndio, explosão, queda de raio, fumaça e queda de aeronave",
    obrigatoria: true,
    desc: "Cobertura básica obrigatória contra sinistros estruturais graves.",
  },
  {
    id: "danos_eletricos",
    nome: "Danos elétricos",
    obrigatoria: false,
    desc: "Cobertura para danos em equipamentos por curto-circuito ou variação de energia.",
  },
  {
    id: "vendaval",
    nome: "Vendaval, furacão, ciclone, tornado e granizo",
    obrigatoria: false,
    desc: "Cobertura para fenômenos climáticos severos.",
  },
  {
    id: "rc_familiar",
    nome: "Responsabilidade civil familiar",
    obrigatoria: false,
    desc: "Cobre danos involuntários causados a terceiros pelo segurado ou seus familiares.",
  },
  {
    id: "perda_aluguel",
    nome: "Perda ou pagamento de aluguel",
    obrigatoria: false,
    desc: "Garante o aluguel em caso de sinistro que impeça o uso do imóvel.",
  },
];
...
const ASSISTENCIAS = [
  ...
  {
    id: "basic",
    nome: "Emergencial Básico",
    desc: "Chaveiro, vidraceiro e encanador em emergências.",
  },
  {
    id: "essential",
    nome: "Emergencial Essencial",
    desc: "Inclui o básico + elétrica, hidráulica e desentupimento.",
  },
  {
    id: "complete",
    nome: "Emergencial Completo",
    desc: "Cobertura completa de manutenção emergencial.",
  },
  {
    id: "complete_pet",
    nome: "Emergencial Completo + PET",
    desc: "Cobertura completa com assistência veterinária para PETs.",
  },
];
...
const PAGAMENTOS = [
  ...
  },
  {
    ...
    label: "Pix",
    icon: QrCode,
    desc: "Pagamento à vista com confirmação imediata.",
  },
  {
    id: "boleto" as const,
    label: "Boleto",
    icon: ReceiptIcon,
    desc: "Pagamento à vista via boleto bancário.",
  },
];
...
function calcularPercentualIncendio(
...
function FinalizarPage() {
  ...
  const [tipoSeguroImovel, setTipoSeguroImovel] = useState<"residencial" | "comercial">(
    "residencial",
  );
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if ((data as any).insurance_coverages?.length)
          setCoberturas((data as any).insurance_coverages);
        if ((data as any).insurance_assistance) setAssistencia((data as any).insurance_assistance);
        if ((data as any).insurance_commission_pct != null)
  ...
  const documentosMeta =
    consulta?.documentos && typeof consulta.documentos === "object" ? consulta.documentos : {};
  ...
  const taxaAtivacao =
    (consulta?.activation_fee_enabled ?? extrasSummary?.activation_fee_enabled)
      ? Number(consulta?.activation_fee_amount ?? extrasSummary?.activation_fee_amount) || 0
      : 0;
  const pinturaTotal =
    (consulta?.external_painting_enabled ?? extrasSummary?.external_painting_enabled)
      ? Number(consulta?.external_painting_total ?? extrasSummary?.external_painting_total) || 0
      : 0;
  const pinturaMensal =
    (consulta?.external_painting_enabled ?? extrasSummary?.external_painting_enabled)
      ? Number(
          consulta?.external_painting_installment ?? extrasSummary?.external_painting_installment,
  ...
  const valorComissao = useMemo(
    () => premioAnual * (comissaoPct / 100),
    [premioAnual, comissaoPct],
  );
  const totalFinal = useMemo(
    () => premioAnual + taxaAtivacao + pinturaTotal,
    [premioAnual, taxaAtivacao, pinturaTotal],
  );
  ...
  const premioIncendioAnual = useMemo(
    () => aluguel * (percentualIncendio / 100),
    [aluguel, percentualIncendio],
  );
  ...
  if (loading || !consulta) {
    return (
      <DashboardLayout>
        <div className="p-10 text-sm text-neutral-500">Carregando...</div>
      </DashboardLayout>
    );
  }
      ...
      <Dialog open={termosOpen} onOpenChange={setTermosOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              NOX Fiança — Contrato de seguro imobiliário (180 Seguros)
            </DialogDescription>
          </DialogHeader>
          <div className="text-sm text-neutral-700 space-y-3 leading-relaxed">
            <p>
              <strong>1. Objeto.</strong> O presente seguro tem por objeto garantir, dentro dos
              limites contratados, prejuízos diretos causados aos bens segurados em decorrência de
              riscos cobertos.
            </p>
            <p>
              <strong>2. Coberturas.</strong> As coberturas selecionadas durante a contratação
              compõem a apólice. Coberturas obrigatórias não podem ser removidas.
            </p>
            <p>
              <strong>3. Imóveis aceitos.</strong> A apólice cobre imóveis em alvenaria. Imóveis de
              madeira ou com estrutura predominantemente madeireira não são aceitos.
            </p>
            <p>
              <strong>4. Pagamento.</strong> O pagamento é realizado conforme a forma escolhida
              (cartão de crédito, Pix ou boleto). Alterações posteriores podem exigir nova cotação.
            </p>
            <p>
              <strong>5. Restrições.</strong> Caso seja identificada característica impeditiva para
              emissão do seguro, a apólice poderá não ser emitida e a proposta poderá ser cancelada.
            </p>
            <p>
              <strong>6. Vigência.</strong> A apólice vigora por 12 meses a contar da data de
              ativação pelo inquilino.
            </p>
            <p>
              <strong>7. Sinistros.</strong> Sinistros devem ser comunicados em até 5 dias úteis
              pelos canais oficiais da NOX.
            </p>
          </div>
          <DialogFooter>
            <Button
...
function ResumoPropostaLoft(p: any) {
  const {
    ...
    setComissaoPct,
    valorComissao,
    totalSeguroImobiliario,
    parcelaSeguro,
    percentualIncendio,
    premioIncendioMensal,
    premioIncendioAnual,
    aluguel,
    condominio,
    taxas,
  ...
  const tipoPagador =
    consulta?.payment_type === "imobiliaria"
      ? "Recorrência via Imobiliária"
      : "Recorrência via Inquilino";
  const solicitacao =
    String(id).slice(0, 8).replace(/\D/g, "") || numeroProposta.replace("NOX-", "");
  ...
  const mensalidadeFinal =
    incendioPagamento === "parcelado"
      ? mensalidadeBaseComPintura + premioIncendioMensal
      : mensalidadeBaseComPintura;
  const pagamentoInicial =
    (incendioPagamento === "a_vista" ? premioIncendioAnual : 0) + taxaAtivacao;
  const valorCobradoAgora = mensalidadeFinal + pagamentoInicial;
  const pagamentoSelecionado = PAGAMENTOS.find((item) => item.id === pagamento);
  const pagamentoOpcoes = ["boleto", "pix", "credit_card"]
    .map((id) => PAGAMENTOS.find((item) => item.id === id))
    .filter(Boolean) as typeof PAGAMENTOS;
  const irParaPagamento = () => {
    setPagamentoAberto(true);
    window.setTimeout(
      () =>
        document
          .getElementById("pagamento-proposta")
  ...
  const [cartao, setCartao] = useState(emptyCardForm);
  const [pagamentoAsaasResultado, setPagamentoAsaasResultado] =
    useState<NormalizedAsaasPayment | null>(null);
  const [modalAsaasAberto, setModalAsaasAberto] = useState(false);
  const [cronogramaResultado, setCronogramaResultado] = useState<CronogramaResultado | null>(null);
  const [modalCronogramaAberto, setModalCronogramaAberto] = useState(false);
  const pagandoRef = useRef(false);

  // Quando o responsavel pelo pagamento e o inquilino e a forma e boleto, a
  // contratacao gera as 12 mensalidades de uma vez (asaas-create-installment-
  // plan) em vez de uma cobranca unica - nenhuma outra combinacao muda.
  ...
  async function handlePagarComAsaas() {
    ...
    if (!nome || !email || !telefone || !documento) {
      toast.error(
        "Complete os dados do inquilino (nome, e-mail, telefone e CPF/CNPJ) antes de pagar.",
      );
      return;
    }
    ...
    try {
      ...
      const body: any = {
        ...
        amount: Number(valorCobradoAgora.toFixed(2)),
      };
      ...
      <div className="mx-auto w-full max-w-[780px] px-4 py-10 space-y-5">
        ...
        <section className="rounded-md border border-slate-300 bg-white p-6">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button
              variant="outline"
              onClick={onCancelar}
              className="h-11 rounded-md border-neutral-900 px-6 text-neutral-950"
            >
              Cancelar proposta
            </Button>
          </div>
        ...
        <ResumoBox title="Dados da locação" onEdit={onEditarDados}>
          <ResumoRow
            ...
            value={<strong>{consulta?.imovel_subtipo || imovel?.tipo || "Residencial"}</strong>}
          />
        ...
        <ResumoBox title="Endereço do imóvel" onEdit={onEditarDados}>
          <div className="space-y-5 text-sm text-neutral-950">
            ...
            <div>
              ...
              <p>
                {[
                  consulta?.imovel_endereco,
                  consulta?.imovel_numero,
                  consulta?.imovel_bairro,
                  consulta?.imovel_cidade,
                ]
                  .filter(Boolean)
                  .join(", ")}
        ...
        <ResumoBox title="Dados do inquilino" onEdit={onEditarDados}>
          <div className="rounded-sm border border-slate-300">
            <span className="inline-flex bg-sky-700 px-2 py-1 text-xs font-bold text-white">
              Pagador
            </span>
        ...
        <section className="rounded-md border border-slate-300 bg-white p-6">
          ...
          <div className="space-y-6">
              ...
              <div className="relative flex min-h-[280px] items-center justify-between gap-4 overflow-hidden rounded-md border border-slate-300 bg-white p-8">
                <div className="max-w-[60%]">
                  ...
                  <p className="mt-2 text-sm text-slate-600">
                    Cobertura básica obrigatória contra sinistros estruturais graves.
                  </p>
                  <p className="mt-4 text-2xl font-bold text-neutral-950">
                    {fmt(premioIncendioMensal)}{" "}
                    <span className="text-sm font-normal text-slate-500">/mês</span>
                  </p>
                  <p className="mt-1 text-xs text-slate-500">
                    Total anual de {fmt(premioIncendioAnual)}, dividido em 12x de{" "}
                    {fmt(premioIncendioMensal)} ({percentualIncendio}% do aluguel).
                  </p>
                </div>
                <img
            ...
            <div className="flex gap-3 rounded-md border border-yellow-300 bg-yellow-50 p-4 text-sm text-neutral-800">
              ...
              <p>
                Caso seja identificado que o imóvel possui alguma dessas características, a apólice
                do seguro não será emitida, e a proposta será cancelada.
              </p>
            </div>

            {pagamentoAberto && (
              <section
                id="pagamento-proposta"
              ...
              >
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-600">
                    Escolha a forma de pagamento e como cobrar o seguro incêndio.
                  </p>
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span
                            className={`h-4 w-4 rounded-full border-2 ${selected ? "border-neutral-900 bg-yellow-400" : "border-slate-300"}`}
                          />
                        </div>
                        <div>
                ...
                {ehParceladoInquilino && (
                  <div className="rounded-md border border-slate-300 bg-slate-50 p-4">
                    <p className="text-sm font-bold text-neutral-950">
                      O inquilino paga por boleto: 12 boletos mensais serão gerados agora
                    </p>
                    <p className="mt-1 text-xs text-slate-600">
                      Cada mensalidade vira uma cobrança independente, visível em "Minhas Faturas".
                    </p>
                    <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-slate-700 sm:grid-cols-3">
                      {cronogramaPreview.map((parcela) => (
                        <div key={parcela.installmentNumber}>
                          {parcela.installmentNumber}/12 — {MESES_PT[parcela.referenceMonth - 1]}/
                          {parcela.referenceYear} · {formatDateBr(parcela.dueDate)}
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {pagamento === "credit_card" && (
                  <div className="rounded-md border border-slate-300 bg-slate-50 p-4">
                    <p className="mb-3 text-sm font-bold text-neutral-950">Dados do cartao</p>
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      <Input
                        value={cartao.holderName}
                        onChange={(e) =>
                          setCartao((prev) => ({ ...prev, holderName: e.target.value }))
                        }
                      ...
                      />
                      <Input
                        value={cartao.holderEmail}
                        onChange={(e) =>
                          setCartao((prev) => ({ ...prev, holderEmail: e.target.value }))
                        }
                      ...
                      />
                      <Input
                        value={cartao.holderAddressNumber}
                        onChange={(e) =>
                          setCartao((prev) => ({ ...prev, holderAddressNumber: e.target.value }))
                        }
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-slate-600">
                      Cobra {fmt(premioIncendioAnual)} separado e mantém a mensalidade da fiança em{" "}
                      {fmt(premioMensal)}.
                    </p>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-slate-600">
                      Soma {fmt(premioIncendioMensal)} por mês na parcela da fiança.
                    </p>
                  </button>
                </div>
                ...
                <div className="rounded-md bg-black p-5 text-white">
                  ...
                  <div className="mt-4 space-y-2 text-sm">
                    <div className="flex justify-between gap-4">
                      <span className="text-neutral-200">Mensalidade da fiança</span>
                      <strong>{fmt(premioMensal)}</strong>
                    </div>
                    <div className="flex justify-between gap-4">
                      <span className="text-neutral-200">Seguro incêndio</span>
                      <strong>
                        {incendioPagamento === "parcelado"
                          ? `${fmt(premioIncendioMensal)} / mês`
                          : `${fmt(premioIncendioAnual)} à vista`}
                      </strong>
                    </div>
                    {taxaAtivacao > 0 && (
                      <div className="flex justify-between gap-4">
                        <span className="text-neutral-200">Taxa de ativação</span>
                        <strong>{fmt(taxaAtivacao)}</strong>
                      </div>
                    )}
                    {pinturaMensal > 0 && (
                      <div className="flex justify-between gap-4">
                    ...
                    <div className="border-t border-neutral-700 pt-3">
                      <div className="flex items-end justify-between gap-4">
                        ...
                        <strong className="text-2xl text-yellow-400">
                          {fmt(mensalidadeFinal)}
                        </strong>
                      </div>
                      {pagamentoInicial > 0 && (
                        <p className="mt-1 text-xs text-neutral-300">
                          Pagamento inicial/à vista: {fmt(pagamentoInicial)}.
                        </p>
                      )}
                      <p className="mt-1 text-xs text-neutral-300">
                        Valor cobrado agora:{" "}
                        <strong className="text-white">{fmt(valorCobradoAgora)}</strong>.
                      </p>
                      {pagamentoSelecionado && (
                        <p className="mt-1 text-xs text-neutral-300">
                          Forma selecionada: {pagamentoSelecionado.label}.
                        </p>
                      )}
                    </div>
                  </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20 border-t border-slate-200 bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex w-full max-w-[780px] flex-col gap-1">
          <div className="flex w-full items-center justify-between gap-3">
            ...
            <Button
              onClick={pagamentoAberto ? handlePagarComAsaas : irParaPagamento}
          ...
          {!seguroConcluido && (
            <p className="text-xs text-neutral-500">
              Aceite os Termos e Condições para seguir para o pagamento.
            </p>
          )}
          {pagamentoAberto && !pagamento && (
            <p className="text-xs text-neutral-500">
              Escolha uma forma de pagamento para enviar a proposta.
            </p>
          )}
        </div>
      </div>
  ...
  return (
    <section className="rounded-md border border-slate-300 bg-white p-6">
      <div className="mb-4 flex items-center justify-between border-b border-slate-300 pb-4">
        ...
        {onEdit && (
          <button type="button" onClick={onEdit} className="text-sm text-emerald-700 underline">
            Editar dados
          </button>
        )}
      </div>
...
function TipoImovelCard(p: {
  ...
  return (
    <div
      className={`rounded-md border p-5 ${p.selected ? "border-slate-400" : "border-slate-300"}`}
    >
      <button
        type="button"
        onClick={p.onClick}
      ...
      >
        ...
        <span
          className={`h-4 w-4 rounded-full border ${p.selected ? "border-emerald-700 bg-emerald-500" : "border-neutral-900 bg-white"}`}
        />
      </button>
      <Select
        value={p.selectValue}
      ...
      <ol className="grid grid-cols-1 sm:grid-cols-4 gap-2 sm:gap-3">
        {ETAPAS.map((e, i) => {
          ...
          return (
            <li
              key={e.key}
              className={`flex items-center gap-2 p-2 rounded-lg ${active ? "bg-yellow-50 border border-yellow-200" : ""}`}
            >
              <span
              ...
              <span
                className={`text-xs font-bold leading-tight ${active ? "text-neutral-900" : done ? "text-neutral-700" : "text-neutral-400"}`}
              >
                {e.label}
              </span>
            </li>
...
function StatusBadge({ status, substatus }: { status?: string; substatus?: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span
        className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-black uppercase tracking-wide border ${s.cls}`}
      >
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-500">
          Confira os dados da locação antes de avançar para a configuração do seguro.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 auto-rows-fr">
        ...
        <Card icon={<MapPin size={14} />} title="Endereço do imóvel">
          ...
          <Linha
            label="Endereço"
            value={`${consulta?.imovel_endereco ?? ""} ${consulta?.imovel_numero ?? ""}`}
          />
          <Linha label="Complemento" value={consulta?.imovel_complemento} />
          <Linha label="Bairro" value={consulta?.imovel_bairro} />
          <Linha
            label="Cidade/UF"
            value={`${consulta?.imovel_cidade ?? ""}/${consulta?.imovel_estado ?? ""}`}
          />
        </Card>
        ...
        <Card icon={<Building2 size={14} />} title="Tipo de pagamento">
          <div
            className={`p-3 rounded-xl border-2 ${consulta?.payment_type === "imobiliaria" ? "border-yellow-400 bg-yellow-50" : "border-neutral-200"} mb-2`}
          >
            <p className="text-xs font-black flex items-center gap-1">
              <Building2 size={14} /> Via imobiliária
            </p>
            <p className="text-[11px] text-neutral-600">
              Imobiliária recebe boleto único mensal com todos os contratos.
            </p>
          </div>
          <div
            className={`p-3 rounded-xl border-2 ${consulta?.payment_type === "inquilino" ? "border-yellow-400 bg-yellow-50" : "border-neutral-200"}`}
          >
            <p className="text-xs font-black flex items-center gap-1">
              <User size={14} /> Via inquilino
            </p>
            <p className="text-[11px] text-neutral-600">
              O inquilino paga a fiança diretamente à NOX.
            </p>
          </div>
        </Card>
        ...
        <Card icon={<FileCheck2 size={14} />} title="Documentos enviados">
          {documentos.length === 0 && (
            <p className="text-xs text-neutral-400">Nenhum documento enviado.</p>
          )}
          ...
          {contratoAssinadoPendente && (
            <div className="mt-3 flex gap-2 rounded-lg border border-yellow-300 bg-yellow-50 p-3 text-xs text-neutral-800">
              ...
              <span>
                <strong>Pendência:</strong> enviar o contrato de locação assinado e atualizado após
                finalizar.
              </span>
            </div>
          )}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-500">
          Escolha coberturas, assistências e configuração comercial do seguro.
        </p>
      </div>
        ...
        <div className="space-y-6">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-1">
                      ...
                      {c.obrigatoria ? (
                        <span className="inline-flex items-center gap-1 text-[10px] font-black bg-neutral-900 text-yellow-400 px-2 py-0.5 rounded-full">
          ...
          <Card icon={<Sparkles size={14} />} title="Assistências">
            <p className="text-xs text-neutral-500 mb-3">
              Escolha qual assistência será adicionada ao seguro.
            </p>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span
                        className={`w-4 h-4 rounded-full border-2 ${sel ? "border-neutral-900 bg-yellow-400" : "border-neutral-300"}`}
                      />
                    </div>
          ...
          <Card icon={<CreditCard size={14} />} title="Comissão">
            <p className="text-xs text-neutral-500 mb-3">
              Escolha a % de comissão que deseja aplicar neste contrato de seguro.
            </p>
            ...
            <p className="text-[11px] text-neutral-400 mt-2">
              Esse valor é incluso no custo final ao inquilino.
            </p>
          </Card>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-500">
          Selecione como o cliente deseja pagar a contratação do seguro.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6">
        <div className="space-y-4">
          ...
          <div className="p-3 rounded-lg bg-blue-50 border border-blue-200 text-xs text-neutral-700 flex gap-2">
            ...
            <span>
              <strong>Atenção:</strong> caso seja necessário alterar a forma de pagamento depois,
              uma nova cotação poderá ser criada.
            </span>
          </div>
          ...
          <div className="p-4 rounded-xl bg-yellow-50 border border-yellow-300 text-sm text-neutral-800 flex gap-3">
            ...
            <div>
              ...
              <p className="text-xs leading-relaxed">
                Caso seja identificado que o imóvel possui alguma característica impeditiva para
                emissão do seguro, a apólice poderá não ser emitida e a proposta poderá ser
                cancelada.
              </p>
            </div>
          </div>
        ...
        <ResumoSticky
          ...
          pinturaTotal={pinturaTotal}
          pagamento={pagamento}
          valorBaseImovel={Number(imovel?.valor_aluguel) || 0}
        />
      </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-500">
          Confirme todas as informações antes de enviar para ativação do inquilino.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 auto-rows-fr">
        <Card icon={<FileText size={14} />} title="Resumo geral">
          ...
          <Linha
            label="Criada em"
            value={new Date(consulta?.created_at).toLocaleDateString("pt-BR")}
          />
        </Card>
        ...
        <Card icon={<MapPin size={14} />} title="Endereço">
          <p className="text-sm text-neutral-700">
            {consulta?.imovel_endereco}, {consulta?.imovel_numero}
            <br />
            {consulta?.imovel_bairro} — {consulta?.imovel_cidade}/{consulta?.imovel_estado}
            <br />
        ...
        <Card icon={<CheckCircle2 size={14} />} title="Confirmações">
          ...
          <Linha
            ...
            value={
              consulta?.terms_accepted_at
                ? new Date(consulta.terms_accepted_at).toLocaleString("pt-BR")
                : "—"
            }
          />
        </Card>
        ...
        <Card icon={<Clock size={14} />} title="Histórico da proposta">
          <ol className="space-y-2">
            <li className="text-xs flex items-center gap-2">
              <CheckCircle2 size={12} className="text-emerald-500" /> Proposta criada
            </li>
            <li className="text-xs flex items-center gap-2">
              <CheckCircle2 size={12} className="text-emerald-500" /> Dados preenchidos
            </li>
            <li className="text-xs flex items-center gap-2">
              <CheckCircle2 size={12} className="text-emerald-500" /> Seguro configurado
            </li>
            <li className="text-xs flex items-center gap-2">
              <Circle size={12} className="text-neutral-400" /> Revisão pronta para envio
            </li>
            {historico.map((h: any) => (
              <li key={h.id} className="text-xs flex items-center gap-2">
                <CheckCircle2 size={12} className="text-emerald-500" /> {h.descricao}
              </li>
            ))}
          </ol>
        </Card>
        ...
        <Card icon={<CreditCard size={14} />} title="Valor final">
          ...
          <div className="border-t border-neutral-100 mt-2 pt-2">
            <Linha
              label="Total final"
              value={<strong className="text-neutral-900">{fmt(totalFinal)}</strong>}
            />
          </div>
        </Card>
      </div>
      ...
      <div className="flex justify-between gap-3 pt-2">
        ...
        <Button
          onClick={onEnviar}
          disabled={enviando}
          className="bg-yellow-400 text-neutral-900 hover:bg-yellow-300 font-black"
        >
...
function EtapaEnviada(p: any) {
  ...
  const pagLabel =
    PAGAMENTOS.find((x) => x.id === pagamento)?.label ?? consulta?.insurance_payment_method_label;
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 auto-rows-fr">
        <Card icon={<FileText size={14} />} title="Detalhes do envio">
          ...
          <Linha
            label="Enviada em"
            value={
              consulta?.proposta_enviada_em
                ? new Date(consulta.proposta_enviada_em).toLocaleString("pt-BR")
                : "—"
            }
          />
        </Card>
      ...
      <div className="flex flex-col sm:flex-row gap-2 justify-center">
        <Button variant="outline" onClick={onVerDetalhes}>
          Ver detalhes da proposta
        </Button>
        <Button
          onClick={onVoltarContratos}
          className="bg-neutral-900 text-yellow-400 hover:bg-neutral-800"
        >
    ...
    <div className="lg:sticky lg:top-6 h-fit bg-neutral-900 text-white rounded-2xl p-5 space-y-3">
      ...
      {pagamento === "credit_card" && (
        <p className="text-xs text-neutral-300">
          Em 12x de <strong className="text-white">{fmt(parcela12)}</strong> no cartão de crédito
        </p>
      )}
      {pagamento && pagamento !== "credit_card" && (
        <p className="text-xs text-neutral-300">
          ...
          <strong className="text-yellow-400">
            {PAGAMENTOS.find((p) => p.id === pagamento)?.label}
          </strong>
        </p>
      )}
      ...
      <div className="border-t border-neutral-700 pt-3 space-y-1.5 text-xs">
        <div className="flex justify-between">
          <span className="text-neutral-400">Prêmio mensal</span>
          <span>{fmt(premioMensal)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-neutral-400">Prêmio anual</span>
          <span>{fmt(premioAnual)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-neutral-400">Comissão</span>
          <span>{comissaoPct}%</span>
        </div>
        <div className="flex justify-between">
          <span className="text-neutral-400">Taxa ativação</span>
          <span>{fmt(taxaAtivacao)}</span>
        </div>
        {pinturaTotal > 0 && (
          <div className="flex justify-between">
        ...
        {valorBaseImovel ? (
          <div className="flex justify-between">
            <span className="text-neutral-400">Aluguel base</span>
            <span>{fmt(valorBaseImovel)}</span>
          </div>
        ) : null}
      </div>
          ...
          {coberturas.slice(0, 4).map((c: string) => {
            ...
            return (
              <li key={c} className="flex items-start gap-1">
                ...
                <span className="text-neutral-200">{cob?.nome}</span>
              </li>
            );
          })}
        </ul>
