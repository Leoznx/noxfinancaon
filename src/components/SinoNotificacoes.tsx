...
import { Bell, DollarSign, CheckCircle, Wallet, Trophy, FileCheck, FileX, Megaphone, Sparkles, X } from "lucide-react";
          ...
          <div className="absolute top-full right-0 mt-3 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-neutral-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="px-5 py-4 border-b border-neutral-50 flex items-center justify-between bg-white">
              <div>
                ...
                {naoLidas > 0 && (
                  <p className="text-[10px] text-yellow-600 font-bold uppercase tracking-widest mt-0.5">{naoLidas} novas mensagens</p>
                )}
              </div>
              <button 
            ...
            <div className="max-h-[400px] overflow-y-auto">
              {carregando ? (
                ...
                <div className="text-center py-12 px-6">
                  ...
                  <p className="text-xs text-neutral-500 mt-1">Você será avisado quando houver novidades na plataforma.</p>
                </div>
              ) : (
                <div className="divide-y divide-neutral-50">
                  {notificacoes.map(notif => {
                    ...
                    return (
                      ...
                      >
                        <div className={`flex-shrink-0 ${cor.fundo} rounded-xl p-2.5 h-fit shadow-sm`}>
                          <Icone className={`w-4 h-4 ${cor.icone}`} strokeWidth={2.5} />
                        </div>
                        
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            <p className="text-sm font-bold text-neutral-900 leading-tight">{notif.titulo}</p>
                            {!notif.lida && (
