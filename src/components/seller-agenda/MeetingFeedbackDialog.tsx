...
export function MeetingFeedbackDialog({ item, onClose, onSubmit }: { item: SellerAppointment | null; onClose: () => void; onSubmit: (feedback: string) => Promise<void> }) {
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && !saving && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <div className="mb-2 grid h-11 w-11 place-items-center rounded-2xl bg-yellow-100 text-yellow-800">
